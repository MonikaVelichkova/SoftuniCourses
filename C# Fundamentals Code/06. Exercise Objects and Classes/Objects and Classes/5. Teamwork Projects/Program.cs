...
namespace _5._Teamwork_Projects
{
    ...
    {        
        static void Main(string[] args)
        {
            ...
            for (int i = 1; i <= n; i++)
            {
                ...
                if (teams.Any(teams => teams.Creator == creatorName))
                {
        ...
        static void PrintValidTeams(List<Team> validTeams)
        {
            foreach (Team validTeam in validTeams)
            {
                ...
                foreach (string currMem in validTeam.Members.OrderBy(m => m))
                {
                    Console.WriteLine($"-- {currMem}");
                }
            }
        }
    ...
    class Team
    {
            public Team(string teamName, string creatorName)
            {
                ...
                //Always remeber to initialize collections in List!!
            }
